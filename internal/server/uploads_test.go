...
func uploadMultipart(t *testing.T, url, filename, content string) envelope {
	t.Helper()
...
func postJSON(t *testing.T, url, body string) envelope {
	t.Helper()
...
func TestUploadAndCreateTaskWithFile(t *testing.T) {
	...
	e := uploadMultipart(t, ts.URL+"/api/uploads", "a.mp3", "FAKE")
	if e.Code != 0 {
	...
	created := postJSON(t, ts.URL+"/api/tasks", body)
	if created.Code != 0 {
...
func TestUploadUnknownFileID(t *testing.T) {
	...
	e := postJSON(t, ts.URL+"/api/tasks", body)
...
func TestUploadStream(t *testing.T) {
	...
	miss, err := http.Get(ts.URL + "/api/uploads/" + uuid.NewString() + "/stream")
	if err != nil {
		t.Fatal(err)
	}
