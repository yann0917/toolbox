...
// 接口按「最小必要」设计（写入/签名读/探活/生命周期），火山 TOS 为首个实现；
// 阿里 OSS、腾讯 COS 等均兼容 S3 协议，后续以 S3 实现接入（New 的 provider 分派已预留）。
...
type Config struct {
	...
	LifecycleDays int    // 生命周期（天），0 = 不设置；仅供设置页「应用规则」动作使用
...
type Client interface {
	...
	// ApplyLifecycle 以「替换同前缀清理规则、保留其他规则」的合并语义应用到期删除规则，
	// 返回人类可读结果描述（供设置页直接展示）。
	ApplyLifecycle(ctx context.Context, days int) (string, error)
}
...
func New(cfg Config) (Client, error) {
	...
	default:
		// s3 通道（OSS/COS/MinIO 等）预留：接口已按最小必要收敛，接实现即可，不改调用方。
		return nil, fmt.Errorf("对象存储 provider %q 暂未支持（当前支持 tos，s3 兼容通道规划中）", cfg.Provider)
	}
}
