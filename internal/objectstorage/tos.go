...
// lifecycleRuleID 本包管理的生命周期规则标识：ApplyLifecycle 只增删改这条规则，
// 桶上用户自建的其他规则原样保留。
const lifecycleRuleID = "toolbox-auto-clean"
...
func (t *tosClient) ApplyLifecycle(ctx context.Context, days int) (string, error) {
	// 无规则时 TOS 返回 404（NoSuchLifecycle 语义），按空规则集处理。
	rules, err := func() ([]tos.LifecycleRule, error) {
		out, gerr := t.cli.GetBucketLifecycle(ctx, &tos.GetBucketLifecycleInput{Bucket: t.bucket})
		if gerr != nil {
			var serr *tos.TosServerError
			if errors.As(gerr, &serr) && serr.StatusCode == 404 {
				return nil, nil
			}
			return nil, gerr
		}
	...
	if err != nil {
		return "", fmt.Errorf("读取桶生命周期规则失败: %w", humanTOSerr(err))
	}

	kept := make([]tos.LifecycleRule, 0, len(rules)+1)
	for _, r := range rules {
	...
	if days > 0 {
		kept = append(kept, tos.LifecycleRule{
			ID:         lifecycleRuleID,
			Prefix:     t.prefix,
			Status:     enum.LifecycleStatusEnabled,
			Expiration: &tos.Expiration{Days: days},
		})
	}
	if _, err := t.cli.PutBucketLifecycle(ctx, &tos.PutBucketLifecycleInput{
		Bucket: t.bucket,
		Rules:  kept,
	}); err != nil {
		return "", fmt.Errorf("写入桶生命周期规则失败: %w", humanTOSerr(err))
	}
	if days > 0 {
		return fmt.Sprintf("已设置：前缀 %s 下对象 %d 天后自动删除（桶上其他规则未受影响）", target, days), nil
	}
	return "已移除本工具的自动清理规则（桶上其他规则未受影响）", nil
}
