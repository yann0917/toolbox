# toolbox JSON 输出契约（v1）

适用范围：CLI `--json` stdout、`toolbox mcp` 工具输出。二者共享同一执行核心，形状完全一致。
...
- **CLI / 脚本**：`toolbox <tool> ... --json`，以退出码判断成败后直接解析 stdout。
- **MCP agent**：`toolbox_*` 工具输出即本契约（JSON 文本内容）。
