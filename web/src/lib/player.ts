...
function ensureAudio(): HTMLAudioElement {
  ...
  el.addEventListener("error", () => usePlayer.setState({ loading: false, playing: false }));
  audio = el;
...
export const usePlayer = create<PlayerState>((set, get) => ({
  ...
  play: (t, autoplay = true) => {
    ...
    set({ track: t, duration: 0, loading: true });
    el.load();
    if (autoplay) {
      void el
        .play()
  ...
  stop: () => {
    ...
    time = 0;
    set({ track: null, playing: false, loading: false, duration: 0 });
  },
}));
