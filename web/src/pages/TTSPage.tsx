...
export default function TTSPage() {
  ...
  /* 音色列表：从 /api/voices 拉取，按 category 分组渲染。
    ...
    mutationFn: () =>
      fetchJSON<{ task_id: string }>("/api/tasks", {
        ...
        body: JSON.stringify({
          provider: "volcengine",
          tool: "tts",
          params: { text, voice: effectiveVoice, format, speed_ratio: speed, volume_ratio: volume },
        }),
      }),
    onSuccess: (d) => {
      setTaskId(d.task_id);
  ...
  return (
    <>
      <PageHeader
        title="语音合成"
        description="文本转语音，音色来自接口列表，语速与音量可调"
        actions={
          <Link
            to="/history"
